...
def pull_order_to_array_bw(order, canvas, nails, strength):
    for pull_start, pull_end in zip(order, order[1:]):
        rr, cc, val = line_aa(nails[pull_start][0], nails[pull_start][1], nails[pull_end][0], nails[pull_end][1])
...
nails = create_circle_nail_positions(shape, nail_step=NAIL_STEP, r1_multip=RADIUS1_MULTIPLIER, r2_multip=RADIUS2_MULTIPLIER)
...
for i in range(1, PULL_AMOUNT+1):
    idx, best_nail_position, best_cumulative_improvement = find_best_nail_position(current_position, nails, str_pic, orig_pic, STRENGTH)
    ...
    chord = hypot(best_nail_position[0] - current_position[0], best_nail_position[1] - current_position[1])
    thread_length_px += chord

    current_position = best_nail_position

    if i % SNAPSHOT_INTERVAL == 0:
        image_dimens = SIDE_LEN, SIDE_LEN
        blank = init_canvas(image_dimens, black=WB)
...
total_length_cm = thread_length_px * (1.0 / SIDE_LEN) * 70  # approximate for 70cm board

OUTPUT_FOLDER = os.path.join(os.path.dirname(INPUT_FILE), "Output1")
...
# ✅ Save pull sequence text file to Output folder
